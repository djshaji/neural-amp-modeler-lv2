...
namespace NAM {
	...
	void Plugin::process(uint32_t n_samples) noexcept
	{
		...
		float desiredInputLevel = 0.0f ;
		//~ if (ports.input_level)
		desiredInputLevel = powf(10, *(ports.input_level) * 0.05f);
		...
		{
			//~ printf ("have model\n");
			model->process(ports.audio_out, ports.audio_out, n_samples);
			model->finalize_(n_samples);

			if (model->HasLoudness())
			{
				// Normalize model to -18dB
				modelLoudnessAdjustmentDB = -18 - model->GetLoudness();
			}
		}
		...
		float desiredOutputLevel = 0.0f ;
		//~ if (ports.output_level)
		desiredOutputLevel = powf(10, (*(ports.output_level) + modelLoudnessAdjustmentDB) * 0.05f);
